...
class Program
{
    ...
    static void TestAllMethods()
    {
        ...
        Console.WriteLine($"1. Square(3) = {Square(3)}");
        Console.WriteLine($"2. Sum(8, 12) = {Sum(8, 12)}");
        Console.WriteLine($"3. GetStringLength('Grok') = {GetStringLength("Grok")}");

        // Тестирование методов из блока 2 (рекурсия)
        Console.WriteLine($"129. Factorial(6) = {Factorial(6)}");
        Console.WriteLine($"115. Fibonacci(7) = {Fibonacci(7)}");

        // Тестирование методов из блока 3 (перегрузка)
        PrintInfo("Измененная строка");
        PrintInfo(50);
        PrintInfo(3.14159);

        // Тестирование методов из блока 4 (опциональные параметры)
        Console.WriteLine($"301. SQL: {BuildSQLQuery(table: "Customers", where: "Age > 21")}");
    ...
    static void GetSubsetsHelper174<T>(List<T> set, int index, List<T> current, List<List<T>> subsets)
    {
        ...
        GetSubsetsHelper174(set, index + 1, current, subsets);
    ...
    // Задача 201: Создайте перегруженные методы для вывода информации (строка, число)
    ...
    {
        Console.WriteLine($"Int info: {info}");
    }

    static void PrintInfo(double info)
    {
        Console.WriteLine($"Double info: {info}");
    }
